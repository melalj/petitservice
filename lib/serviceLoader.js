...
function serviceLoader() {
  ...
  chain.publisher = (amqUrlRaw, publisherExchanges) => {
    ...
    const amqUrl = `${amqUrlRaw}${(amqUrlRaw.match(/\?/) ? '&' : '?')}heartbeat=20`;
    promises.publisher = () => {
  ...
  chain.coworkers = (options) => {
    ...
    promises.coworkers = () => {
      const amqUrl = `${options.amqUrl}${(options.amqUrl.match(/\?/) ? '&' : '?')}heartbeat=20`;
      coworkersApp = options.consumer();
      return coworkersApp.connect(amqUrl, { keepAlive: true, timeout: 15, noDelay: true })
      .then(() => {
